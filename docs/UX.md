...
Public pages use a compact brand header with the Dictionary Challenge wordmark linking home and one primary challenge action that starts the child flow when no quiz is in progress and returns to an unfinished challenge when one exists. Quiz routes use a compact toolbar with status and a Home action so the question flow stays focused on the dictionary activity.
