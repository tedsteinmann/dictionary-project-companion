...
export function renderSponsors(config, { compact = false } = {}) {
  ...
  return `<section class="sponsors ${compact ? 'sponsors-compact' : ''}" aria-labelledby="sponsors-title">
    <h2 id="sponsors-title">Made possible by</h2>
    <div class="sponsor-grid">${sponsors.map((sponsor) => `<article class="sponsor">
      ...
      <a href="${escapeHtml(sponsor.url)}" rel="noreferrer">Learn more<span class="sr-only"> about ${escapeHtml(sponsor.title)}</span> <span aria-hidden="true">↗</span></a>
    </article>`).join('')}</div>
  </section>`;
}
